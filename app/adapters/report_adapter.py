    ...
    def generate(
        ...
        try:
            ...
            if not field_validation.passed:
                artifact.status = "errors"
                logger.error(f"[{run_id}] Validation failed with {len(field_validation.errors)} errors (including control totals)")

                # Build minimal manifest for failed validation
                artifact.manifest = {
                    "run_id": run_id,
                    "tenant_id": tenant_id,
                    "tenant_name": mapper.tenant_name,
                ...
                # Write validation artifacts so operator can see errors
                elapsed = time.time() - start_time
                artifact.generation_time_seconds = elapsed
                bundle_paths = artifact.to_bundle(run_dir)
                logger.info(f"[{run_id}] Field validation artifacts written: {list(bundle_paths.keys())}")

                # Return artifact so operator can see errors
                return artifact
            ...
            logger.info(f"[{run_id}] Report generation complete in {elapsed:.2f}s")
